...
export function createIconEndpoints(providers) {
    const byId = new Map(providers.map((provider)=>[
            provider.id,
            provider
        ]));
    const getProvider = (url)=>byId.get(providerIdFromURL(url));
    return [
        {
            async handler (req) {
                ...
                try {
                    ...
                    return json({
                        error: 'Unable to load icon catalog'
                    }, 500);
                }
            },
            method: 'get',
        ...
        },
        {
            async handler (req) {
                ...
                if (!body.names.every((name)=>typeof name === 'string')) {
                    return json({
                ...
                if (names.length === 0 || names.length > MAX_BATCH_SIZE || names.some((name)=>name.length === 0 || name.length > MAX_NAME_LENGTH)) {
                    return json({
                        error: `names must contain 1-${MAX_BATCH_SIZE} valid icon names`
                    }, 400);
                }
                if (body.weight !== undefined && typeof body.weight !== 'string') {
                    return json({
                        error: 'weight must be a string'
                    }, 400);
                }
                try {
